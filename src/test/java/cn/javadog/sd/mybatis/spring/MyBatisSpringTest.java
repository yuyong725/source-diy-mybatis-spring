...
class MyBatisSpringTest extends AbstractMyBatisSpringTest {
  ...
  /*开始使用 SqlSessionUtils 代替 MyBatis 的 API*/

  /**
   * 和上面的{@link #testMyBatisAPI()} 一样，实际并未交给Spring管理
   */
  @Test
  void testSpringAPI() {
  ...
  /**
   * Spring的事务，非Spring的事务工厂，Spring管理数据源。
   * 只要数据源不再由mybatis管理，就会GG
   */
  @Test
  void testNonSpringTxFactoryWithTx() throws Exception {
    ...
    try {
      // spring开启事务了, 也会将数据源绑定到Spring的事务
...
//      fail("should not be able to get an SqlSession using non-Spring tx manager when there is an active Spring tx");
  ...
  /**
   * Spring的事务，非Spring的事务工厂，MyBatis管理事务
   */
  @Test
